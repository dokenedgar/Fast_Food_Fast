	...
	.then((data) => { 
		...
		orders.forEach( function(element, index) {
			...
			element.order.forEach( function(elementf, index) {
				...
				status.innerHTML = '<select><option value='+element.status+'>'+element.status+'</option><option value=""> </option><option value="Accepted">Accepted</option><option value="fiat">Rejected</option><option value="audi">Completed</option></select>'
